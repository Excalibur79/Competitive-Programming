...
void solve() {
    ...
    if ((arr[startIdx] != start && hash[arr[startIdx]] != 2) ||
        (arr[endIdx] != end && hash[arr[endIdx]] != 2)) {
        cout << -1 << endl;
        return;
    }
    ...
    while (!q.empty()) {
        ...
        vector<pll> neighbourCells = {{curr.first + 1, curr.second},
        ...
        for (pll neighbour : neighbourCells) {
            ll neighbourIdx =
                lower_bound(arr.begin(), arr.end(), neighbour) - arr.begin();
            if (arr[neighbourIdx].first != neighbour.first) continue;
            if (!visited[neighbour] && (arr[neighbourIdx] == neighbour ||
                                        hash[arr[neighbourIdx]] == 2)) {
